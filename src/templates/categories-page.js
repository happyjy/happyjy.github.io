...
const categoryPage = ({ data, pageContext }) => {
  ...
  // console.log('### categoies-page.js: ', { categories, categoryPostCounts });
  // console.log({data, categories, categoryPostCounts});
  // console.log({edges});
  const newCategoryPostCounts = Object.keys(categoryPostCounts).map(function(
    key
  ) {
  ...
  return (
    <Layout pageTitle="Category">
          ...
          {newCategoryPostCounts.map(category => (
            ...
            <li key={category[0]}>
              ...
              <Button
                ...
                href={`#${category[0]}`}
              >
                {category[0]} <Badge color="info">{category[1]}</Badge>
              </Button>
            </li>
          ))}
        </ul>
      ...
      <div>
        <div className="categories">
          <ul className="categoryPost">
            {newCategories.map((category, index) => (
              // style={{ marginBottom: '10px'}}
              <li key={category}>
                <h2 id={category}> # {category}</h2>
                <CategoryList key={index} category={category} edges={edges} />
              </li>
            ))}
            {/* // {
            // edges.map(edge => {
